...
\tag{1.5}
\end{align}
\noindent\framebox[\textwidth][c]{codes/bm1.py}
...
\begin{tikzpicture}[scale=1.4]
    ...
    \node[below] at (1.5,-0.65) {Length of each segment: 1};
...
\noindent\framebox[\textwidth][c]{codes/bm3.py}
...
Using the Laplace transform feedback relation for a unity negative feedback system, the closed-loop transfer function is:
\[
...
Substitute $G(s)$:
\[
\frac{C(s)}{R(s)} = \frac{\frac{25}{s(s+5)}}{1 + \frac{25}{s(s+5)}} = \frac{25}{s^2 + 5s + 25}
\]
...
\[
s^2 + 5s + 25 = \left(s + \frac{5}{2}\right)^2 + \frac{75}{4} = (s + 2.5)^2 + \left(\frac{5\sqrt{3}}{2}\right)^2
\]
...
\]
\[
C(s) = \frac{10}{\sqrt{3}} \cdot \frac{\frac{5\sqrt{3}}{2}}{\left(s + \frac{5}{2}\right)^2 + \left(\frac{5\sqrt{3}}{2}\right)^2}
\]
...
Correct option: (B) is underdamped


\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figs/bm6.png}
    \caption{c(t) vs time}
    \label{c(t) vs time}
\end{figure}

\noindent\framebox[\textwidth][c]{codes/bm6.py}
...
\end{align}
\begin{align}
\text{Therefore, the correct answer is (B)}
\end{align}
\noindent\framebox[\textwidth][c]{codes/bm7.py}
...
\end{figure}
\noindent\framebox[\textwidth][c]{codes/bm8.py}
...
\end{align}
\begin{align}
\text{Hence, the correct answer is (b) 11}
\end{align}

\noindent\framebox[\textwidth][c]{codes/bm9.py}
...
\end{figure}
\noindent\framebox[\textwidth][c]{codes/bm10a.py}
...
Hence, the correct answer is (a) 500
...
\end{figure}
\noindent\framebox[\textwidth][c]{codes/bm10.py}
...
Hence, the correct answer is (b) 12.5



\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figs/bm11.png}
    \caption{Power vs time}
    \label{power vs time}
\end{figure}
\noindent\framebox[\textwidth][c]{codes/bm11.py}
...
Hence, the correct answer is (d) 64
...
\end{figure}
\noindent\framebox[\textwidth][c]{codes/bm12.py}
...
Let $\mathcal{L}\{y(x)\} = Y(s)$. Applying the Laplace Transform derivative property:
\[
...
Group $Y(s)$ terms together:
\[
(s^2 - 0.25) Y(s) = s + 1
\]

\[
Y(s) = \frac{s + 1}{s^2 - 0.25} = \frac{s + 1}{\left(s - \frac{1}{2}\right)\left(s + \frac{1}{2}\right)}
\]
...
\[
A = \lim_{s \to 0.5} (s - 0.5) Y(s) = \frac{0.5 + 1}{0.5 + 0.5} = 1.5 = \frac{3}{2}
\]
\[
B = \lim_{s \to -0.5} (s + 0.5) Y(s) = \frac{-0.5 + 1}{-0.5 - 0.5} = -0.5 = -\frac{1}{2}
\]
...
\noindent\framebox[\textwidth][c]{codes/bm13.py}
...
\[
Z_p(\omega) = \frac{R_2 \cdot Z_C(\omega)}{R_2 + Z_C(\omega)} = \frac{R_2 \left(-\frac{j}{\omega C}\right)}{R_2 - \frac{j}{\omega C}} = \frac{-j R_2}{\omega R_2 C - j}
\]

Multiplying numerator and denominator by the complex conjugate $(\omega R_2 C + j)$:
\[
Z_p(\omega) = \frac{-j R_2 (\omega R_2 C + j)}{(\omega R_2 C - j)(\omega R_2 C + j)} = \frac{R_2 - j \omega R_2^2 C}{1 + (\omega R_2 C)^2}
\]

The total input impedance $Z_{\text{in}}(\omega)$ in series with $R_1$ is:
\[
Z_{\text{in}}(\omega) = R_1 + Z_p(\omega) = \left( R_1 + \frac{R_2}{1 + (\omega R_2 C)^2} \right) - j \left( \frac{\omega R_2^2 C}{1 + (\omega R_2 C)^2} \right)
\]
...
\[
\angle Z_{\text{in}}(\omega) = -\tan^{-1}\left( \frac{\frac{\omega R_2^2 C}{1 + (\omega R_2 C)^2}}{R_1 + \frac{R_2}{1 + (\omega R_2 C)^2}} \right) = -\tan^{-1}\left( \frac{\omega R_2^2 C}{R_1(1 + (\omega R_2 C)^2) + R_2} \right)
\]

\vspace{0.5em}
\text{Numerical Evaluation at $\omega = 1000\text{ rad/s}$}

Substituting $R_1 = 3\text{ k}\Omega$, $R_2 = 1\text{ k}\Omega$, $C = 1\,\mu\text{F}$, and $\omega = 1000\text{ rad/s}$:
...
\[
Z_p = \frac{1 - j(1)}{1 + (1)^2} = 0.5 - j0.5\text{ k}\Omega
\]

Total impedance:
\[
Z_{\text{in}} = 3 + (0.5 - j0.5) = 3.5 - j0.5\text{ k}\Omega
\]

Evaluating Magnitude $|Z_{\text{in}}|$:
\[
|Z_{\text{in}}| = \sqrt{(3.5)^2 + (-0.5)^2} = \sqrt{12.25 + 0.25} = \sqrt{12.5} \approx 3.5355\text{ k}\Omega \approx 3.5\text{ k}\Omega
\]

Evaluating Phase $\angle Z_{\text{in}}$:
\[
\angle Z_{\text{in}} = \tan^{-1}\left(\frac{-0.5}{3.5}\right) = -\tan^{-1}\left(\frac{1}{7}\right) \approx -8.13^\circ
\]

\text{Correct Answer: (b) 3.5}
...
\end{figure}
\noindent\framebox[\textwidth][c]{codes/bm14.py}
...
\begin{align*}
\text{System } U: & \quad y(t) = x(t)^2 + 1 \\
\text{System } V: & \quad y(t) = x(t + 1)
\end{align*}


\subsection*{System $U$: $y(t) = x(t)^2 + 1$}

\text{1. Linearity Check (Additive \& Homogeneous):}
\begin{itemize}
    \item \text{Additivity:} Let $y_1(t) = x_1(t)^2 + 1$ and $y_2(t) = x_2(t)^2 + 1$.
    For input $x_3(t) = x_1(t) + x_2(t)$, the output is:
    \[
    y_3(t) = [x_1(t) + x_2(t)]^2 + 1 = x_1(t)^2 + 2x_1(t)x_2(t) + x_2(t)^2 + 1
    \]
    Since $y_1(t) + y_2(t) = x_1(t)^2 + x_2(t)^2 + 2 \neq y_3(t)$, the system is \text{not additive}.

    \item \text{Homogeneity:} For input $a \cdot x(t)$, the output is:
    \[
    y_{scaled}(t) = [a \cdot x(t)]^2 + 1 = a^2 x(t)^2 + 1 \neq a \cdot y(t)
    \]
    Thus, it is \text{not homogeneous}.
\end{itemize}
\text{Conclusion:} System $U$ is \text{nonlinear}.

\vspace{0.5em}
\text{2. Causality Check:}
\begin{itemize}
    \item At $t = 0$: $y(0) = x(0)^2 + 1$.
    \item The output at $t = 0$ depends only on the present input $x(0)$.
\end{itemize}
\text{Conclusion:} System $U$ is \text{causal}.



\subsection*{System $V$: $y(t) = x(t + 1)$}

\text{1. Linearity Check (Additive \& Homogeneous):}
\begin{itemize}
    \item \text{Additivity:} Let $y_1(t) = x_1(t + 1)$ and $y_2(t) = x_2(t + 1)$.
    For input $x_3(t) = x_1(t) + x_2(t)$:
    \[
    y_3(t) = x_1(t + 1) + x_2(t + 1) = y_1(t) + y_2(t) \quad \text{(Satisfied)}
    \]

    \item \text{Homogeneity:} For input $a \cdot x(t)$:
    \[
    y_{scaled}(t) = a \cdot x(t + 1) = a \cdot y(t) \quad \text{(Satisfied)}
    \]
\end{itemize}
\text{Conclusion:} System $V$ is \text{linear}.

\vspace{0.5em}
\text{2. Causality Check:}
\begin{itemize}
    \item At $t = 0$: $y(0) = x(0 + 1) = x(1)$.
    \item The output at $t = 0$ depends on the future input $x(1)$.
\end{itemize}
\text{Conclusion:} System $V$ is \text{noncausal}.





\noindent\text{Option (a): System $U$ is nonlinear; System $V$ is linear}
    \begin{itemize}
        \item System $U$ fails additivity and homogeneity $\rightarrow$ nonlinear.
        \item System $V$ satisfies additivity and homogeneity $\rightarrow$ linear.
    \end{itemize}

\noindent\text{Option (b): System $U$ is causal; System $V$ is noncausal}
    \begin{itemize}
        \item For System $U$, $y(0) = x(0)^2 + 1$ (depends on current input) $\rightarrow$ causal.
        \item For System $V$, $y(0) = x(1)$ (depends on future input) $\rightarrow$ noncausal.
       
    \end{itemize}

  \noindent\text{Option (c): System $U$ is noncausal; System $V$ is causal}
    \begin{itemize}
        \item Incorrect based on the $y(0)$ check above.
    \end{itemize}

  \noindent\text{Option (d): System $U$ is nonlinear; System $V$ is nonlinear}
    \begin{itemize}
        \item System $V$ is linear, making this statement incorrect.
    \end{itemize}



\noindent Correct options are \text{(a)} and \text{(b)}.
...
The linear convolution $y[n] = h[n] * x[n]$ has a total length of $L = M + N - 1 = 3 + 2 - 1 = 4$ points (spanning $n = 0, 1, 2, 3$).

\vspace{0.5em}
\text{Construct the Convolution (Toeplitz) Matrix}

Linear convolution can be represented as a matrix-vector product $\mathbf{y} = \mathbf{H} \mathbf{x}$, where $\mathbf{H}$ is a $(M + N - 1) \times N = 4 \times 2$ lower-triangular Toeplitz matrix formed by shifting $h[n]$ downwards:
...
1 & 0 \\[4pt] 
\frac{1}{2} & 1 \\[4pt] 
\frac{1}{3} & \frac{1}{2} \\[4pt] 
0 & \frac{1}{3} 
\end{bmatrix}
\]

The input vector $\mathbf{x}$ is:
\[
\mathbf{x} = \begin{bmatrix} x[0] \\[4pt] x[1] \end{bmatrix} = \begin{bmatrix} 1 \\[4pt] 1 \end{bmatrix}
\]
...
\[
\begin{bmatrix} y[0] \\[4pt] y[1] \\[4pt] y[2] \\[4pt] y[3] \end{bmatrix} = 
\begin{bmatrix} 
1 & 0 \\[4pt] 
\frac{1}{2} & 1 \\[4pt] 
\frac{1}{3} & \frac{1}{2} \\[4pt] 
0 & \frac{1}{3} 
\end{bmatrix}
\begin{bmatrix} 1 \\[4pt] 1 \end{bmatrix}
\]
...
\begin{itemize}
    ...
    \item \text{Option (c):} $y[k] = 0 \text{ for all } k \ge 3 \quad \text{--- Incorrect (since } y[3] = \frac{1}{3} \neq 0\text{)}$
    \item \text{Option (d):} $y[k] = 0 \text{ for all } k \ge 4 \quad \text{--- Correct}$
\end{itemize}

\text{Correct Options: (b) and (d)}
...
\noindent\framebox[\textwidth][c]{codes/bm17.py}
...
\noindent\framebox[\textwidth][c]{codes/bm18.py}
...
Rounding off to the nearest integer gives: -14
...
\vspace{0.5em}
\text{Time-Domain Differential Equation}

For the series RLC circuit driven by a sinusoidal voltage source $v(t) = V_m \cos(\omega t)$, Kirchhoff's Voltage Law (KVL) gives:
\[
R i(t) + L \frac{di(t)}{dt} + \frac{1}{C} \int i(t) dt = v(t)
\]

Assuming steady-state sinusoidal response $i(t) = I_m \cos(\omega t - \phi)$:

\vspace{0.5em}
\text{Phasor / Frequency-Domain Impedance}
...
The magnitude of the circuit impedance as a function of angular frequency $\omega$ is:
\[
...
Resonance occurs at frequency $\omega_0$ where the reactive components cancel each other ($X_L = X_C$), minimizing $|Z(\omega)|$:
\[
...
The root-mean-square (RMS) current $I_{\text{rms}}$ at resonance is:
\[
I_{\text{rms}}(\omega_0) = \frac{V_{\text{rms}}}{|Z(\omega_0)|} = \frac{V_{\text{rms}}}{R}
\]
...
\noindent\framebox[\textwidth][c]{codes/bm21.py}
...
The general iterative update formula is defined as:
\[
...
\text{Answer for $x_1$ after ONE iteration: 2.36}
...
\end{figure}
\noindent\framebox[\textwidth][c]{codes/bm22a.py}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figs/bm23.png}
    \caption{Strength vs Time}
    \label{strength of implanted suture}
\end{figure}

\noindent\framebox[\textwidth][c]{codes/bm23.py}
...
\begin{align}
\text{(C) USA and Canada together have less than 50\% of the medals awarded to the nations in the above table.}
\tag{25.2}
\end{align}

\noindent\framebox[\textwidth][c]{codes/bm25.py}
...
\tag{27.4}
\end{align}
\noindent\framebox[\textwidth][c]{codes/bm26.py}
