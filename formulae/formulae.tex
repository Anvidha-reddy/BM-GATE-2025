...
\begin{align}
Z_p(\omega) &= \frac{R_2 \cdot Z_C(\omega)}{R_2 + Z_C(\omega)}\\
& = \frac{R_2 \left(-\frac{j}{\omega C}\right)}{R_2 - \frac{j}{\omega C}}\\
...
\begin{align}
-e^x &= e^{x} \cdot e^{i(2k+1)\pi}\\
&= e^{x + i(2k+1)\pi} \label{e^x}
\end{align}
...
\text{Causality}: a system is causal if the output at time $t_0$
